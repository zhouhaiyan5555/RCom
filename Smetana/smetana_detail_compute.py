...
def get_combination_name(files, excluded_file):
    ...
    return f"12mix-{identifier}_m9_ananerobic_exclu_bigg"
...
def main():
    ...
    command_task1 = f"smetana {' '.join(xml_files)} -o 12mix_m9_ananerobic_exclu_bigg --flavor bigg --solver cplex --molweight --mediadb M9_anaerobic_media_db.tsv --exclude inorganic_compounds.tsv -d"
    ...
    for combination in xml_combinations:
        for excluded_file in xml_files:
            if excluded_file not in combination:
                ...
                command_task2 = f"smetana {' '.join(combination)} -o {output_name} --flavor bigg --solver cplex --molweight --mediadb M9_anaerobic_media_db.tsv --exclude inorganic_compounds.tsv -d"
